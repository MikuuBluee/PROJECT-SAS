...
exports.createOrder = (orderData, callback) => {
    ...
    db.query(queryOrders, [orderData.user_id, orderData.total_harga, 'pending'], (err, result) => {
        ...
        const queryItems = 'INSERT INTO order_items (order_id, product_id, quantity, subtotal) VALUES ?';
        db.query(queryItems, [items], (err2) => {
            if(err2) return callback(err2);

            const chartIds = orderData.chart_ids;
            if(chartIds.length > 0){
                const queryDeleteChart = `DELETE FROM charts WHERE id IN (${chartIds.map(() => '?').join(',')})`;
                db.query(queryDeleteChart, chartIds, (err3) => {
